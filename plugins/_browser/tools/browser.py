...
class Browser(Tool):
        ...
        try:
            if action == "open":
                ...
                result = await runtime.call(
                    "screenshot_file", browser_id, quality=quality, full_page=full_page, path=path
                )
            elif action == "list":
                ...
                result = await runtime.call(
                    "detail", browser_id, await self._resolve_ref(runtime, browser_id, ref, selector, action)
                )
            elif action == "click":
                resolved_ref = await self._resolve_ref(
                    runtime, browser_id, ref, selector, action, required=not self._has_coordinates(x, y)
                )
                if resolved_ref is None and self._has_coordinates(x, y):
                    ...
                    result = await runtime.call(
                        "click", browser_id, resolved_ref, modifiers=modifiers, focus_popup=focus_popup
                    )
                else:
                    result = await runtime.call("click", browser_id, resolved_ref)
            elif action == "type":
                resolved_ref = await self._resolve_ref(
                    runtime, browser_id, ref, selector, action, required=False
                )
                ...
                result = await runtime.call(
                    "submit", browser_id, await self._resolve_ref(runtime, browser_id, ref, selector, action)
                )
            elif action in {"type_submit", "typesubmit"}:
                result = await runtime.call(
                    "type_submit", browser_id,
                    await self._resolve_ref(runtime, browser_id, ref, selector, action), text
                )
            elif action == "scroll":
                result = await runtime.call(
                    "scroll", browser_id, await self._resolve_ref(runtime, browser_id, ref, selector, action)
                )
                ...
                result = await runtime.call(
                    "hover", browser_id, ref=ref, x=x, y=y,
                    offset_x=offset_x, offset_y=offset_y,
                )
            elif action == "double_click":
                result = await runtime.call(
                    "double_click", browser_id, ref=ref, x=x, y=y,
                    button=button or "left", modifiers=modifiers,
                    offset_x=offset_x, offset_y=offset_y,
                )
            elif action == "right_click":
                result = await runtime.call(
                    "right_click", browser_id, ref=ref, x=x, y=y,
                    modifiers=modifiers, offset_x=offset_x, offset_y=offset_y,
                )
            elif action == "drag":
                result = await runtime.call(
                    "drag", browser_id, ref=ref, target_ref=target_ref,
                    x=x, y=y, to_x=to_x, to_y=to_y,
                    offset_x=offset_x, offset_y=offset_y,
                    target_offset_x=target_offset_x, target_offset_y=target_offset_y,
                )
            elif action == "wheel":
                result = await runtime.call("wheel", browser_id, x, y, delta_x, delta_y)
            elif action == "keyboard":
                result = await runtime.call("keyboard", browser_id, key=key, text=text)
            elif action == "clipboard":
                normalized_clipboard_action = clipboard_action or str(
                    kwargs.get("clipboard_action") or kwargs.get("operation") or event_type or ""
                ).strip().lower()
                result = await runtime.call(
                    "clipboard", browser_id, action=normalized_clipboard_action, text=text
                )
            elif action in {"copy", "cut", "paste"}:
                result = await runtime.call("clipboard", browser_id, action=action, text=text)
            elif action == "set_viewport":
                result = await runtime.call("set_viewport", browser_id, width, height)
            elif action == "select_option":
                result = await runtime.call(
                    "select_option", browser_id,
                    await self._resolve_ref(runtime, browser_id, ref, selector, action),
                    value=value, values=values,
                )
            elif action == "set_checked":
                result = await runtime.call(
                    "set_checked", browser_id,
                    await self._resolve_ref(runtime, browser_id, ref, selector, action),
                    checked=True if checked is None else bool(checked),
                )
            elif action == "upload_file":
                result = await runtime.call(
                    "upload_file", browser_id,
                    await self._resolve_ref(runtime, browser_id, ref, selector, action),
        ...
        *,
        ...
        if selector:
            ...
            if resolved is not None:
                return resolved
            raise ValueError(f"{action} could not resolve selector {selector!r} to a browser ref")
    ...
    def _normalize_keys(keys: list[str] | str | None) -> list[str]:
        ...
        if isinstance(keys, str):
            ...
            raw = [str(keys)]
        aliases = {
            "cmd": "Meta", "command": "Meta", "control": "Control", "ctrl": "Control",
            "escape": "Escape", "esc": "Escape", "meta": "Meta", "option": "Alt",
        ...
        for key in raw:
            ...
            if not value:
                continue
            normalized.append(
                aliases.get(value.lower(), value.upper() if len(value) == 1 and value.isalpha() else value)
            )
        return normalized
    ...
    async def _record_history_screenshot(
        ...
        if not self._screenshot_has_reference(screenshot):
            ...
            try:
                screenshot = await runtime.call(
                    "screenshot_file", target_browser_id,
                    quality=HISTORY_SCREENSHOT_QUALITY, full_page=False, path="",
                )
            except Exception as exc:
                PrintStyle.debug(
        ...
        if local_path:
            ...
            snapshot.update({"ephemeral": bool(ephemeral_ref), "ephemeral_ref": ephemeral_ref})
    ...
    def _browser_id_from_result(result: Any) -> Any:
        ...
        return (
            result.get("id") or result.get("browser_id") or state.get("id")
            or last_interacted_id or (listed_browser or {}).get("id")
        )
